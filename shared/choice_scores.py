...
.. warning::
   Mirror of ``fluiq-workers/evaluator/jobs/helper/choice_scores.py``, identical
   below the module docstring. The API validates a choice set at author time;
   the evaluator applies it to every example. A set that validates here but
   resolves differently there would grade silently wrong.
