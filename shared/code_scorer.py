...
.. warning::
   This file is a byte-for-byte mirror of
   ``fluiq-workers/evaluator/jobs/helper/code_scorer.py``, below the module
   docstring. The API and the evaluator are separate deployables that cannot
   share code at runtime, and a scorer that validates here but behaves
   differently there would be worse than no validation at all.

   ``fluiq-api/tests/test_code_scorer_mirror.py`` fails if the two drift.
   Edit the evaluator's copy, then re-copy it here.

The API uses this to validate a scorer at author time and to run the editor's
"test" button; the evaluator uses it to score every example of a run.
